...
def submit_quiz(request, attempt_id):
    ...
    for sa in attempt.answers.select_related('question').all():
        q = sa.question
        selected = request.POST.get(f'q_{q.id}', '')  # get selected option
...
def start_quiz(request, quiz_id):
    ...
    return render(request, 'quiz/take_quiz.html', {'quiz': quiz, 'attempt': attempt, 'questions': quiz.questions.all()})
...
def result_view(request, attempt_id):
    attempt = get_object_or_404(QuizAttempt, id=attempt_id, student=request.user)
    return render(request, 'quiz/result.html', {'attempt': attempt, 'answers': attempt.answers.all()})
